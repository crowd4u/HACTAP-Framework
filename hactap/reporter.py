...
class Reporter:
    def __init__(self, params: Namespace = None) -> None:
        ...
        self.report['experiment_id'] = self._get_experiment_id(params)
        self.report['started_at'] = self._get_timestamp()
        self.logs: List = []
    ...
    def finalize(self, assignment_log: List) -> None:
        ...
        with open(filename, 'wb') as file:
            self.report['finished_at'] = self._get_timestamp()
    ...
    def _get_experiment_id(self, args: Namespace) -> str:
...
class EvalAIReporter(Reporter):
    def finalize(self, assignment_log: List) -> None:
        group_dir = './results/{}/report_of_ai_workers/'.format(self.group_id)
        os.makedirs(group_dir, exist_ok=True)
