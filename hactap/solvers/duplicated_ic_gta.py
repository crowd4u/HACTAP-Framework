...
class DuplicatedICGTA(solvers.IntersectionalClusterCTA, solvers.GTA):
    ...
    def run(self) -> Tasks:
        ...
        while not self.tasks.is_completed:
            ...
            task_cluster_candidates += self.list_task_clusters_with_id()
    ...
    def list_task_clusters_with_id(self) -> List[TaskCluster]:
        task_clusters = []

        for index, _ in enumerate(self.ai_workers):

            task_clusters.extend(
                self.create_task_cluster_from_ai_worker_with_id(index)
            )
